...
Game::Game() : window(sf::VideoMode(1600, 800), "SFML Game", sf::Style::Titlebar | sf::Style::Close), 
            ...
            selectionSort(dataGen), mergeSort(dataGen), isRunning(true), isDragging(false), timePerFrame(sf::seconds(1.0f / 10.0f)){
    Init();
}
...
void Game::Update(sf::Time deltaTime) {
    ...
    if(visualiser.GetRun() && visualiser.GetSelectionSort() && !selectionSort.IsSorted()){
        
        selectionSort.SortStep();
        
    }

    if(visualiser.GetRun() && visualiser.GetMergeSort() && !mergeSort.IsSorted()){
...
void Game::Render() {
    ...
    // std::string string = "Index of minimum element: " + std::to_string(selectionSort.GetPosIndex());
    ...
    // sf::Text text1;
    // text1.setFont(assetManager.GetFont("font1"));
    // std::string string1 = "start index: " + std::to_string(selectionSort.GetPosStart());
    // text1.setString(string1);
    // text1.setCharacterSize(20);
    // text1.setFillColor(sf::Color::White);
    // text1.setPosition(200.f, 60.f);
    
    // if(visualiser.GetRun() && visualiser.GetSelectionSort() ){
